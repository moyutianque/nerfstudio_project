...
@dataclass
class ShapeNerfDataParserConfig(DataParserConfig):
    """Nerfstudio dataset config"""

    _target: Type = field(default_factory=lambda: ShapeNerfData)
...
@dataclass
class ShapeNerfData(DataParser):
    """ShapeNerfData DatasetParser"""

    config: ShapeNerfDataParserConfig
    downscale_factor: Optional[int] = None
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        mask_filenames = []
        depth_filenames = []
        normal_filenames = [] # NOTE: add by zehao
        sem_filenames = []    # NOTE: add by zehao
        poses = []
        ...
        for frame in meta["frames"]:
            ...
            img_idx = frame["file_path"].split('/')[-1].split('.')[0]
            # NOTE: changed by zehao
            depth_fname = self._get_fname(f"depth/{img_idx}.npy", data_dir)
            depth_filenames.append(depth_fname)

            # NOTE: add by zehao
            normal_fname = self._get_fname(f"normal/{img_idx}.npy", data_dir)
